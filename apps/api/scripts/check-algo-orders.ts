...
async function main() {
  ...
  for (const acct of await prisma.exchangeAccount.findMany({ where: { isActive: true } })) {
    ...
    const a = acct.exchangeId === 'binance' ? new BinanceAdapter({ apiKey: k, apiSecret: s, accountId: acct.id })
      : acct.exchangeId === 'gateio' ? new GateioAdapter({ apiKey: k, apiSecret: s, accountId: acct.id })
      : new OkxAdapter({ apiKey: k, apiSecret: s, passphrase: p, accountId: acct.id });
    try {
