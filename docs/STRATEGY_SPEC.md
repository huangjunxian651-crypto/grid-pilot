...
### 1.4 分层算法单止损，保留反弹恢复能力

在主网格下方设置一段"止损缓冲区"，通过多个条件触发单（算法单）分层减仓，而非在单一价位一次性全平。若价格短暂跌破后反弹，残余仓位可直接受益，省去重新建仓的手续费。
...
**当前问题**：GtcThreshold 在代码中是硬编码的（0.1%），与配置字段 `ExcessProfitMultiplier` 没有连接。这意味着当交易所手续费变化（比如升级到 VIP 档位）或切换交易所时，必须手动修改代码，而不是修改配置。详见第十二章优化建议。
