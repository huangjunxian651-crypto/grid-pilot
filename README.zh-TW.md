...
交易所自帶的網格機器人，本質是「一次性批次掛單、掛上不動、等市場來撞單」。GridPilot 的核心差異是**用程式模擬一個有經驗的盯盤交易員**：

| 維度 | 交易所原生網格 | GridPilot |
|------|----------------|-----------|
| **下單方式** | 批次靜態掛單，掛出後不再移動 | 自動化盯盤：每次行情更新都重新判斷後才動態下單/改單/撤單，任意時刻不一定有掛單 |
| **手續費** | 不區分主動/被動成交 | 三區間定價：POC 區掛 Maker 單（省 ~0.03%）、GTC 區特許吃單鎖定超額利潤、熔斷區拒絕下單 |
| **建倉時機** | 進區間立即開倉 | 追蹤建倉：價格進區間先追低點，確認反彈後才建倉，避免一開倉就站在高位被套 |
| **停損** | 單一價位一次性平倉 | 分層演算法單緩衝減倉，短暫跌破又反彈時殘餘倉位直接受益，省去重建手續費 |
| **行情適應** | 固定單一區間 | 多段不重疊區間，價格進入哪段就啟用哪段，其餘休眠 |

1. **先觀察後決策的盯盤思維**：系統每次收到行情才判斷當前價與目標價的關係——條件不利就停手觀望，條件有利才在最優價卡位，價格大幅跳躍時還能捕獲超出網格步長的額外價差。
2. **Maker 優先 / Taker 特許 / 不利熔斷**：預設掛 Post-Only Maker 單吃更低費率；只有當額外利潤蓋過吃單成本、機會稍縱即逝時才主動吃單；當前價比目標買價更貴時直接拒單，避免高買低賣。
3. **追蹤建倉**：避免頂部開倉被套。
4. **分層演算法單停損**：保留反彈恢復能力。
5. **多段價格區間**：價格去哪裡，策略就跟到哪裡。

> 完整策略原理見 [`docs/STRATEGY_SPEC.md`](docs/STRATEGY_SPEC.md)。

## ✨ 功能概覽

- **多段價格區間**：預配置多個不重疊區間（如 2000–2600、2600–3200），價格進哪段啟用哪段
- **追蹤建倉**：追蹤低點確認反彈後才建倉
- **三區間動態定價**：POC 區 Maker、GTC 區鎖超額利潤、熔斷區拒單
- **分層停損緩衝**：主網格下方多檔演算法條件單分層減倉
- **即時 WebSocket 推送**：Ticker / 成交 / 狀態機事件即時同步前端
- **多交易所支援**：統一介面卡介面，相容 Binance、Gate.io、OKX
- **多語言介面**：內建 12 種語言

## 💰 看懂手續費，註冊時用邀請碼省錢（贊助商 rebateto.me）

手續費由交易所收取，**GridPilot 一分不抽**。同一筆交易，掛單（Maker）≈0.02%、吃單（Taker）≈0.05%。在槓桿與高頻網格下，手續費會被悄悄放大，日積月累並不小——GridPilot 預設替你掛 Maker 單，每筆約省 0.03%，只有機會稍縱即逝時才主動吃單。
...
2. **配置網格**：選擇交易對與價格區間，設定主網格格數、步長、每格數量、槓桿、停損緩衝、追蹤建倉參數。
3. **啟動機器人**：進入追蹤建倉 → 執行，前端即時顯示行情、掛單、成交與狀態機。
4. **監控與收尾**：觸發止盈端停止加倉收尾；觸發停損緩衝分層減倉。
...
| `activationPrice` / `trailingCallbackRate` | 追蹤建倉啟用價 / 回調幅度 |
