...
原生網格價格一進區間就立即開倉。GridPilot **追蹤建倉**：先跟著低點走，反彈 0.2% 確認企穩才進場——不抄底、不站崗。

**行情每秒都在變，你的訂單為什麼掛上去就不再動？**
GridPilot 每次行情更新都重新決策：該撤就撤、該改就改，任意時刻盤口上可能一張單都沒有；價位不利寧可拒單也不追價，能掛 Maker（0.02%）絕不白交 Taker（0.05%）。

**價格一跳 5–10 USDT，你的網格除了乾瞪眼還能做什麼？**
靜態掛單只能吃到網格線上的死價格。GridPilot 在價差超過吃單手續費 2 倍時主動市價吃單，把超出網格步長的跳躍價差鎖進口袋——實測訂單簿越「毛糙」的交易所，超額收益越高。
...
| **成交品質** | 靜態單只能吃到網格線價格，跳躍行情的超額價差擦肩而過 | 追買一/賣一卡最優價；價差超 2× 手續費時主動吃單鎖定超額利潤；價位不利直接拒單 |
| **建倉時機** | 進區間立即開倉 | 追蹤建倉：追低點、確認反彈 0.2% 後才建倉 |
