...
ネイティブグリッドは価格がレンジに入った瞬間に即座に建玉します。GridPilot の**トラッキングエントリー**は、まず安値に追随し、0.2%（デフォルト値、調整可能）の反発で底固めを確認してからエントリー——底値を当てにいかず、高値掴みの棒持ちもしません。（損失方向にアクティベーションウィンドウへ入ってきたときのみトラッキングします。それより深い位置から利確方向にレンジへ戻ってきた場合は、トラッキングをスキップしてそのまま実行を開始します。）

**相場は毎秒動いているのに、なぜあなたの注文は出したまま微動だにしないのか？**
GridPilot は相場が更新されるたびに判断し直します。キャンセルすべきときはキャンセルし、修正すべきときは修正し、ある瞬間には板に注文が 1 枚もないことすらあります。価格が有利なら板を追いかけて最良価格に張り付き、不利なら発注自体を拒否します。Maker（0.02%）で置けるのに無駄に Taker（0.05%）を払うことはありません。この追いかけには下方リスクがありません。価格がグリッド価格を超えてさらに下落すれば、追随してさらに安く買い、上のラインを割らずに反発すれば元の価格で約定できます——標準的な「ギャンブラーの破産」モデル（gambler's ruin）で試算すると、本当に約定を逃す確率はごくわずかです——超過利益はタダで手に入るもので、逃しても損はありません。

**価格が一気に 5〜10 USDT 跳んだとき、あなたのグリッドは指をくわえて眺める以外に何ができますか？**
静的な注文はグリッドライン上の固定価格しか取れません。GridPilot は価格がグリッドの理論価格からしきい値（デフォルト 0.1% ≈ Taker 手数料の 2 倍、調整可能）を超えて乖離したとき、能動的に成行で約定してグリッドのステップ幅を超えるジャンプの超過値幅をポケットに収めます——実測では、板がざらついている取引所ほど超過リターンが高くなります。
...
| **約定品質** | 静的注文はグリッドラインの価格しか取れず、ジャンプ時の超過値幅は取り逃す | 板の最良価格に張り付き、約定価格は理論価格より不利になりません。価格が理論価格からしきい値（デフォルト ≈手数料の 2 倍）を超えて乖離したら、能動的に成行で超過利益を確定。不利な価格では発注自体を拒否 |
| **建玉タイミング** | レンジ進入で即座に建玉 | トラッキングエントリー：損失方向に入ってきたとき安値に追随し、0.2%（デフォルト）の反発を確認してから建玉 |
...
手数料は取引所が徴収するもので、**GridPilot は一切抜きません**。同じ 1 回の取引でも、Maker（指値）は約 0.02%、Taker（成行）は約 0.05% です。日常のグリッド注文はどちら側も Maker で通るため、日常のグリッド約定では手数料に差はありません。GridPilot の手数料上の優位は三つの決定的な場面に現れます——①エントリー：レンジに入った瞬間に成行で建玉するのではなく、反発確認を待って指値でエントリー。②ストップロス：ワンクリックの成行全決済ではなく、グリッドごとに指値で減玉。③食い付き：超過値幅が手数料を確実に上回るときだけ Taker を特別に許可。
...
2. **グリッドを設定**：取引ペアと方向（ロング／ショート）を選び、利確価格のアンカー、メイングリッドのマス数／ステップ幅、1 マスあたりの数量、レバレッジ、ストップロスバッファ、トラッキングエントリーのパラメータを設定します（ボックスの境界は利確価格アンカー + マス数・ステップ幅から自動導出されます）。
3. **ボットを起動**：トラッキングエントリーに入り → 実行すると、フロントエンドが相場、注文、約定、ステートマシンをリアルタイムで表示します。
4. **監視と仕上げ**：価格が利確側に到達すると、グリッドが 1 マスずつ決済して自然に仕上がり、建玉がゼロになったところで利確して終了します。ストップロスバッファ域に入れば、グリッドごとに動的に減玉します。
...
| `direction` | 方向：LONG（ロング）/ SHORT（ショート） |
...
| `isolationStep` | アイソレーションバンドの幅（デフォルト = ストップロス域のステップ幅） |
| `activationPrice` / `trailingCallbackRate` | レンジ起動価格（デフォルトはメイングリッドの中点）／トラッキングエントリーのコールバック幅 |
