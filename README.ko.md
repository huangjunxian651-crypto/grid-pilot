...
네이티브 그리드는 가격이 구간에 들어서는 순간 바로 포지션을 엽니다. GridPilot은 **추적 진입**을 사용합니다: 저점을 따라가며 기다리다가, 0.2% 반등으로 안정이 확인되어야 비로소 진입합니다——섣불리 바닥을 노리지도, 고점에 물려 서 있지도 않습니다. (가격이 손실 방향으로 활성화 윈도우에 진입할 때만 추적하며, 더 깊은 곳에서 구간 안으로 반등해 돌아오는 경우에는 추적을 건너뛰고 바로 실행을 시작합니다.)

**시세는 매초 바뀌는데, 당신의 주문은 왜 한 번 걸면 그대로 멈춰 있나요?**
GridPilot은 시세가 갱신될 때마다 다시 판단합니다: 취소할 건 취소하고, 정정할 건 정정하며, 어떤 순간에는 호가창에 주문이 한 장도 없을 수도 있습니다. 가격이 유리하면 호가창을 따라가며 최적가를 잡고, 가격이 불리하면 차라리 서킷을 끊어 주문을 거부하며, Maker(0.02%)로 걸 수 있는데 굳이 Taker(0.05%) 수수료를 내는 일은 없습니다. 이 추격에는 하방 리스크가 없습니다: 가격이 그리드 가격을 넘어 계속 하락하면 따라가며 더 싸게 매수하고, 위 라인을 깨지 않고 반등하면 원래 가격에 그대로 체결할 수 있습니다 — 표준 도박사의 파산 모델(gambler's ruin)로 계산하면 실제로 체결을 놓칠 확률은 무시할 수준입니다 — 추가 수익은 공짜로 줍는 것이고, 못 주워도 손해는 아닙니다.

**가격이 한 번에 5–10 USDT씩 점프할 때, 당신의 그리드는 멀뚱히 지켜보는 것 말고 뭘 할 수 있나요?**
정적 주문은 그리드 라인의 죽은 가격밖에 먹지 못합니다. GridPilot은 가격이 그리드 이론가에서 임계값(기본 0.1% ≈ 테이커 수수료의 2배, 조정 가능) 이상 벗어나면 능동적으로 체결해, 그리드 스텝을 뛰어넘는 점프 차익을 주머니에 넣습니다——실측 결과, 오더북이 "거칠"수록 초과 수익은 더 커집니다.
...
| **체결 품질** | 정적 주문은 그리드 라인 가격만 먹을 수 있어, 점프 장세의 초과 차익을 눈앞에서 놓침 | 호가창에 고정해 최적가를 잡고 체결가가 이론가보다 나빠지지 않음; 가격이 이론가에서 임계값(기본 ≈수수료의 2배) 이상 벗어나면 능동 체결로 초과 이익 확정; 가격이 불리하면 서킷을 끊어 주문 거부 |
| **진입 타이밍** | 구간 진입 즉시 포지션 오픈 | 추적 진입: 손실 방향으로 진입할 때 저점을 추적하고, 반등(기본 0.2%)을 확인한 후에야 포지션 구축 |
...
수수료는 거래소가 부과하며, **GridPilot은 한 푼도 떼지 않습니다**. 동일한 거래에서 메이커(Maker)는 약 0.02%, 테이커(Taker)는 약 0.05%입니다. 일상적인 그리드 주문은 사실 양쪽 모두 Maker로 체결되므로, 일상 그리드 체결에서는 수수료 차이가 없습니다. GridPilot의 수수료 우위는 세 가지 결정적 순간에서 드러납니다——①진입: 구간 진입 즉시 시장가로 오픈하는 대신, 반등 확인 후 지정가로 걸어 진입, ②손절: 한 번에 시장가로 전량 청산하는 대신, 칸마다 지정가로 단계적 감축, ③테이커: 초과 차익이 확실히 수수료를 상회할 때만 예외적으로 테이커 체결을 허용합니다.
...
2. **그리드 설정**: 거래쌍과 방향(롱/숏)을 선택하고, 익절가 앵커, 메인 그리드 칸 수/스텝, 칸당 수량, 레버리지, 손절 완충, 추적 진입 파라미터를 설정합니다(박스권 경계는 익절가 앵커 + 칸 수·스텝으로 자동 도출됩니다).
3. **봇 시작**: 추적 진입 → 실행으로 진입하면, 프런트엔드에서 시세, 주문, 체결, 상태 머신을 실시간으로 표시합니다.
4. **모니터링 및 마무리**: 가격이 익절단에 도달하면 그리드가 칸마다 청산하며 자연스럽게 마무리되고, 포지션이 0이 되면 익절로 종료됩니다; 손절 완충 구간에 들어가면 칸마다 동적으로 감축합니다.
...
| `isolationStep` | 격리대 폭(기본값 = 손절 구간 스텝) |
| `activationPrice` / `trailingCallbackRate` | 구간 활성화가(기본값 = 메인 그리드 중점) / 추적 진입 콜백 폭 |
